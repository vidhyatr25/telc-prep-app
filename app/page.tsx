  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 md:py-10">
      <section className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-sm font-bold text-yellow-400">TELC German Courses</p>
          <h1 className="mt-2 text-3xl font-extrabold text-white md:text-5xl">
            Choose your exam course
          </h1>
          <p className="mt-3 max-w-2xl text-gray-400">
            Start with the active A1 course. A2 is being prepared with TELC-style tasks. B1 and B2 are planned for later.
          </p>
        </div>
        ...
        {courseLevels.map((level) => {
          ...
          const CardBody = (
            <CourseCardBody
              level={level}
              progressPct={pct}
              completedLabel={level.id === "a1" ? `${completedUnits}/10 units complete` : preparing ? "Research in progress" : "Coming soon"}
            />
          );

          return active ? (
            <Link key={level.id} href="/learn" className="block">
              {CardBody}
            </Link>
          ) : (
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="card p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase text-gray-500">Current active course</p>
              <h2 className="mt-2 text-2xl font-extrabold text-white">German A1</h2>
              <p className="mt-2 text-sm leading-relaxed text-gray-400">
                Your existing A1 course is unchanged: same units, same lessons, same quizzes, same progress.
              </p>
            </div>
        ...
        <div className="card p-5">
          <p className="text-xs font-bold uppercase text-gray-500">A2 preparation</p>
          <h2 className="mt-2 text-2xl font-extrabold text-white">TELC A2 research added</h2>
          <p className="mt-2 text-sm leading-relaxed text-gray-400">
            A2 is not enabled yet. The app now has an A2 outline and sample TELC-style tasks for listening, reading, writing, and speaking so we can build it properly next.
          </p>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {["Listening", "Reading", "Writing", "Speaking"].map((section) => (
              <div key={section} className="rounded-xl border border-gray-800 bg-gray-950 p-3">
                ...
                <p className="mt-1 text-xs text-gray-500">A2 task scaffold ready</p>
              </div>
            ))}
          </div>
        </div>
    ...
    >
      ...
      <div className="mt-5 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold text-gray-500">{level.subtitle}</p>
        </div>
      ...
      <div className="mt-4 flex items-center justify-between text-xs text-gray-500">
        ...
        <span>{level.lessonCount > 0 ? `${level.lessonCount} lessons` : "planned"}</span>
      </div>
